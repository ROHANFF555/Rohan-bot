...
ফিক্স: `requirements.txt`-এ `fastmcp>=2.0,<3` (v2-লাইন) + `mcp>=1.2.0,<2`
(v1-লাইন)। এই টেস্টটা সেই ধারণাটাই লক করে দেয় — v2-লাইনে এই ৪টা kwarg
কনস্ট্রাক্টরেই গ্রহণযোগ্য, তাই `_build_mcp_server()` এক্সেপশন ছাড়াই সার্ভার
বানাতে পারে।

`mcp` কেন `<2`: main.py আগে `mcp.server.fastmcp` চেষ্টা করে, আর ৪টি kwarg-এর
মধ্যে `transport_security` একমাত্র ওই ক্লাসটিই সাপোর্ট করে। standalone
`fastmcp` v2 (ফলব্যাক) সেটা নেয় না, তাই `mcp` 2.x ইনস্টল হলে ফলব্যাক চালু
হয়ে `PUBLIC_URL` সেট থাকা অবস্থায় আবার বিল্ড ফেল করত (ভিন্ন এররে)।
